        ...
        if(authorizationHeader != null && authorizationHeader.startsWith(MyAuthorizationFilter.HeaderPrefix)){
            try {
                ...
                String accessToken = JWT.create()
                        ...
                        .withExpiresAt(new Date(System.currentTimeMillis() + MyAuthenticationFilter.ACCESS_TOKEN_DURATION))
                        .withIssuer(request.getRequestURL().toString())
                        .withClaim(MyAuthenticationFilter.RolesString, new MyUserDetails(user).getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList()))
                        .sign(algorithm);
                ...
                error.put("error_message", "Refresh token expired: "+e.getMessage());
                response.setContentType(APPLICATION_JSON_VALUE);
